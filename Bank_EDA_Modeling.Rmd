...
full_bank_NAs <- full_bank_2 %>% replace_with_na_all(condition = ~.x == "unknown")

basic_eda(full_bank_NAs)

# Create a bird's eye view of missing data using naniar library if missing data exists
vis_miss(full_bank_NAs, cluster = F) + # Wi
labs(title = "NAs in Bank Data from May 2008 - Nov 2010") +
theme(axis.text.x = element_text(angle=90))

gg_miss_upset(full_bank_NAs)
```
...
```{r remove-NAs}
complete_full_bank <- full_bank_NAs[complete.cases(full_bank_NAs),]
...
nlevels(full_bank_2$y)
levels(full_bank_2$y)
```
