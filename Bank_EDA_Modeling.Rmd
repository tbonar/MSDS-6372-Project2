...
cols <- c(colnames(complete_full_bank))
# removing columns that do not provide any value and the response
cols <- cols[cols %!in% c("y","default","lgcampaign","pdays")]
fmla.all <- as.formula(paste("y ~ ", paste(cols, collapse= "+")))
bankModel <- glm(fmla.all, complete_full_bank, family = binomial(link="logit"))
(vif(bankModel)[,3])^2
# removing columns that are highly correlated and with high VIF
cols.lowvif <- cols[cols %!in% c("nr.employed","emp.var.rate")]
...
bankModel.lowvif <- glm(fmla.lowvif, complete_full_bank, family = binomial(link="logit"))
